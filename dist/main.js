...
class App {
    constructor(canvas) {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_2___default()(document).on("keydown", (event) => {
            this.handle_keypress(event);
        });
        jquery__WEBPACK_IMPORTED_MODULE_2___default()('#next').on('click', (event) => {
...
class Scene {
    ...
    resize(canvas, renderingCanvas, GRID_SIZE) {
        this.GRID_SIZEX = Math.floor(canvas.width / GRID_SIZE);
        this.GRID_SIZEY = Math.floor(canvas.height / GRID_SIZE);
    }
    createCells() {
...
class Renderer {
    ...
    async updateGrid() {
        ...
        encoder.copyBufferToBuffer(this.cellStateStorage[1], 
        //this.output,
        0, // Source offset
        this.cellStateStorage[2], 0, // Destination offset
        this.BUFFER_SIZE);
        this.step++; // Increment the step count
        const commands = encoder.finish();
        this.device.queue.submit([commands]);
        await this.cellStateStorage[2].mapAsync(GPUMapMode.READ, 0, // Offset
